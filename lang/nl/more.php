<?php

return [
    'auth-login'  => 'Login',
    'auth-password'  => 'Paswoord',
    'auth-forgot-password'  => 'Paswoord vergeten?',
    'auth-remember-me' => 'Onthoud me',

    'nav-dashboard' => 'Dashboard',
    'nav-installation' => 'Installatie',

    'all' => 'Alle',
    'pedestrian' => 'Voetgangers',
    'bike' => 'Tweewielers',
    'car' => "Auto's",
    'lorry' => 'Grote voertuigen',
    'measurements-last-week' => 'Metingen laatste week',
    'more-data' => 'Meer gegevens...',

    'received-last-data-package-on' => 'Laatste data',

    'looks-like-camera-is-not-sending-data' => 'De Telraam camera stuurt geen data op dit moment.',
    'camera-registered-last-hour' => 'Live metingen laatste uur',
    'objects-per-hour' => 'objecten per uur',
    'camera-shown-after-full-day' => 'The camera is nog niet actief. De cameradata verschijnt wanneer een volledige dag is doorlopen. ',
    'camera-night-or-down' => 'De camera is aangesloten maar detecteert momenteel geen beweging.',

    'hour' => 'uur',

    'loc-det-overview-per-day' => 'Overzicht per dag',
    'loc-det-overview-detail' => 'Overzicht in detail',
    'loc-det-average-24h' => 'Gemiddelde over 24u',
    'loc-det-averige-24h-direction' => 'Gemiddelde over 24u rijrichting',
    'loc-det-percentage-per-type' => 'Percentage per type',
    'share-your-results-on-Facebook' => 'Deel je resultaten op Facebook',
    'share-your-results-on-Twitter' => 'Deel je resultaten op Twitter',

    'set-position' => 'Bevestig je positie door het groene camera icoontje te selecteren die overeenkomt met de kant van de rijbaan van je Telraam.',
    'save-position' => 'Bewaar positie.',

    'check-for-telraam-data' => 'Controle of er daadwerkelijk data binnenkomt bij Telraam voor Camera',

    'camera-not-found' => 'Camera werd nog niet gevonden',
    'camera-is-set-up' => 'Camera is opgezet',
    'camera-is-set-up-but-no-data' => '<strong>Camera is opgezet, maar kreeg nog geen data binnen.</strong> <br>Indien succesvol, zal je hier na maximaal 20 minuten de boodschap krijgen dat jouw toestel inderdaad data verzendt. Als het langer duurt, dan is er iets fout gelopen. <a href=https://telraam.zendesk.com/hc/nl/articles/360026050091-Er-komen-nog-geen-data-door-van-mijn-Telraam target=_blank>Probeer deze stappen</a>',


    'camera-received-data' =>'Camera is opgezet en de server ontving data op',
    'fully-ready' => 'Volledig klaar',
    'did-not-pass-test' => 'Twintig minuten zijn voorbij en we kregen nog geen data package binnen. <a href=https://telraam.zendesk.com/hc/nl/articles/360026050091-Er-komen-nog-geen-data-door-van-mijn-Telraam target=_blank>Probeer deze stappen</a>',
    'we-re-checking-still' => 'We controleren nog',
    'minutes' => 'minut(en)',

    'telraam-site' => 'Telraam website',
    'export-a-excel' => 'Exporteer een excel',
    'with-raw-data' => 'met de ruwe data van de laatste drie maanden.',

    'speed' => "Snelheid",
    'speed-distribution-car' => "Snelheid auto's",
    'speed-remark' => 'Snelheden zijn indicatief. De metingen kunnen 10% afwijken van effectieve snelheden. <br>Hier vind
 je <a href="https://telraam.zendesk.com/hc/nl/articles/360034536711-Hoe-lees-ik-de-snelheidsgrafiek-">meer informatie hoe je deze data kan interpreteren</a>',
    'speed-remark-v85' => 'De V85 is een veelgebruikte indicator in de wereld van de mobiliteit en de verkeersveiligheid, omdat hij geacht wordt representatief te
    zijn voor de snelheid die men op een weg redelijkerwijs kan aanhouden. <a href="https://telraam.zendesk.com/hc/nl/articles/360019460777-Wat-betekent-V85" target="_blank">Lees meer over de V85</a>',

    // 22 feb
    'faq-interpret-data-link' => 'https://telraam.zendesk.com/hc/nl/articles/360026050311-Hoe-moet-ik-de-Telraam-data-interpreteren-',
    'faq-interpret-data-text' => 'Hoe moet ik de Telraam-data interpreteren?',

    // 23 feb
    'remark-upload-picture-dashboard' => 'Deze foto zal worden gebruikt op de publieke website. Vermijd leesbare autoplaten of identificeerbare personen.',

    // 7 mei
    'timerangetoolong' => 'Het verschil tussen de twee data is te lang. Het maximum is zes maanden.',

    // 17 mei
    'no-callibration' => 'Verhouding auto/grote voertuigen is niet correct? Nog even geduld, Telraam heeft tijd nodig om zichzelf te kalibreren. <a href="https://telraam.zendesk.com/hc/nl/articles/360035237672-Telraam-kalibratie-en-de-verhouding-auto-grote-voertuigen">Faq artikel</a>',


    // 7 juni location detail
    'header-data' => 'Data',
    'header-ranking' => 'Ranking',
    'header-street-profile' => 'Straat Profiel',

    // 1 sept location detail
    'all-days' => 'Alle dagen',
    'weekdays-only' => 'Enkel weekdagen',
    'weekend-only'  =>  'Enkel weekend',
    'show-together' => 'Weekdagen vs weekend',
    'date-from' => 'Datum van',
    'date-to' => 'Datum tot',
    'update'    => 'Update',
    'km-hour-unit' => 'km/u',

     // 9 september
    'telraam-installations-location' => 'Telraam installaties op deze locatie',
    'date'          => 'Datum',
    'status'        => 'Status',
    'homeside'      => ' Detecteerbaar door Telraam, huiszijde',
    'opposite'      => 'Detecteerbaar door Telraam, andere kant',

    'not-active'        => 'Niet (meer) actief',
    'counting'          => 'Tellen',
    'backend-failed'    => 'Backend mislukt',
    'problematic'       => 'Problematisch',
    'not-installed'     => 'Niet geïnstalleerd',
    'no-active-found'   => 'Nog geen actieve apparaten gedetecteerd',


];
